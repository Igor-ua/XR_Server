...
validStates=['adrenaline','barbequed','beast_camouflage','beast_immolate','beast_protect','beast_shield','beast_staminaregen','beast_tracking','electrify','fire_shield','human_potion','imobilize','infirmary_heal','magshield','magtowershield','object_fire','officer','officerarea','poisoned','pulsed','rabid','snare','spire_healstructures']
validItems=['ammo_box','arrow_tower_weapon','ballista_weapon','barrel_explode','beast_arcana','beast_behemoth','beast_behemoth_melee','beast_camouflage','beast_charmshrine','beast_entropy_shrine','beast_entropy_spire','beast_entropy1','beast_entropy1b','beast_entropy3','beast_fire_shield','beast_fire_shrine','beast_fire_spire','beast_fire_spire_weapon','beast_fire_trap','beast_fire1','beast_fire2','beast_fire3','beast_gateway','beast_heal','beast_immolate','beast_lair','beast_lair2','beast_lair3','beast_mana_stone','beast_medic','beast_medic_melee','beast_nexus','beast_poison','beast_predator','beast_predator_melee','beast_protect','beast_rabid','beast_recharge','beast_revive','beast_sanctuary','beast_scavenger','beast_scavenger_melee','beast_shield','beast_snare','beast_spire','beast_stalker','beast_stalker_melee','beast_stamina_boost','beast_strata_shrine','beast_strata_spire','beast_strata1','beast_strata2','beast_strata3','beast_sublair','beast_summoner','beast_summoner_weapon','beast_tracking_sense','beast_vampire','beast_worker','catapult_weapon','chem_tower_weapon','dyn_barrel','dyn_column','dyn_crate','dyn_crate_box_plain','dyn_crate_open_peaches','dyn_crate_open_plain','dyn_crate_rect_plain','dyn_ladder','dyn_plank_long','dyn_plank_short','dyn_wood_panel','elec_tower_weapon','foundry','gold_mine','goodiebag','human_adrenaline','human_ammo_pack','human_arrow_tower','human_arsenal','human_ballista','human_bow','human_catapult','human_chemical_factory','human_chemical_tower','human_coilrifle','human_crossbow','human_demo_pack','human_discharger','human_electric_factory','human_electric_tower','human_electrify','human_fluxgun','human_garrison','human_heal','human_imobilizer','human_incinerator','human_landmine','human_launcher','human_legionnaire','human_legionnaire_melee','human_magnetic_factory','human_magnetic_shield','human_magnetic_tower','human_medic','human_medic_melee','human_medkit','human_monastery','human_mortar','human_mortar','human_motion_sensor','human_nomad','human_nomad_melee','human_potion','human_pulsegun','human_relocater','human_relocater_trigger','human_repeater','human_research_center','human_revive','human_savage','human_savage_melee','human_scattergun','human_siege','human_sniperbow','human_stronghold','human_stronghold2','human_stronghold3','human_worker','infirmary','lumber_pile_2500','lumber_pile_2500_stand','lumber_pile_7500','lumber_pile_15000_stand','mana_stone','npc_bearloth','npc_berserker','npc_boulder','npc_buffalo_plains','npc_buffalo_snow','npc_buffalo_waypoint','npc_chiprel','npc_gas','npc_gerkat','npc_hornman','npc_hunchedbeast','npc_kongor','npc_lucy','npc_macaque','npc_mercenary','npc_monkit','npc_mudent','npc_oschore','npc_panda','npc_ronin','npc_shrull','npc_skeleton_guard','npc_skeleton_worker','npc_terror','npc_zizard','pickup_test','pot_explode','pot_tumble','redstone_mine','redstone_mine_small','ruins_frontgate','sawmill','sawmill_small','spawnflag','spire_weapon','stables','wall_prop','wiskeygulch_bridge']
...
def execute(index,cmd):
    ...
    core.ConsolePrint('Ref Command: %s (sent by client %i)\n' % (cmd, index))
    ...
    if cmd.find('help')!=-1:
        ...
        if method.find('kick')!=-1:
            if (server.GetClientInfo(index, INFO_REFSTATUS) == 'normal' and int(core.CvarGetValue('sv_ref_allowkick')) > 0) or (server.GetClientInfo(index, INFO_REFSTATUS) == 'god'):
                method=method.replace('kick','')
                kick(index,str(object))
            else:
               senddenymessage(index)
        if method.find('mute')!=-1:
            if (server.GetClientInfo(index, INFO_REFSTATUS) == 'normal' and int(core.CvarGetValue('sv_ref_allowmute')) > 0) or (server.GetClientInfo(index, INFO_REFSTATUS) == 'god'):
        ...
        if method.find('switchteam')!=-1:
            if (server.GetClientInfo(index, INFO_REFSTATUS) == 'guest') or (server.GetClientInfo(index, INFO_REFSTATUS) == 'normal' and int(core.CvarGetValue('sv_ref_allowswitchteam')) > 0) or (server.GetClientInfo(index, INFO_REFSTATUS) == 'god'):
                method=method.replace('switchteam','')
                switchteam(index,str(object),str(getArgs(method)[0]))
            else:
               senddenymessage(index)
        if method.find('slay')!=-1:
            if (server.GetClientInfo(index, INFO_REFSTATUS) == 'normal' and int(core.CvarGetValue('sv_ref_allowslay')) > 0) or (server.GetClientInfo(index, INFO_REFSTATUS) == 'god'):
...
def giveitem(object,item,slot,ammo):
    if item in validItems:
        if object=='all':
            ...
            server.GameScript(int(sv_utils.getIndexFromName(str(object))),'!give target %s %s %s' % (str(item),str(ammo),str(int(slot)-1)))
...
def spawn(object,item,team):
    if item in validItems:
        if object=='all':
            ...
            server.GameScript(int(sv_utils.getIndexFromName(str(object))),'!spawnobject target %s %s neutral nearby' % (str(item),str(team)))            
    ...
    if object=='all':
        for index in sv_utils.getActiveIndices():
            if target=='home':
                ...
                server.GameScript(int(index),'!teleport target coords %s %s' % (str(target.split('|')[0]),str(target.split('|')[1])))
    elif object.find('team')!=-1:
        object=object.replace('team','')
        for index in sv_utils.getIndicesFromTeam(str(object)):
            if target=='home':
        ...
        server.GameScript(int(136),'!teleport target coords %s %s' % (str(target.split('|')[0]),str(target.split('|')[1])))
    else:
        if target=='home':
            ...
            server.GameScript(int(sv_utils.getIndexFromName(str(object))),'!teleport target coords %s %s' % (str(target.split('|')[0]),str(target.split('|')[1])))
...
def help(index):
    ...
    if (server.GetClientInfo(index, INFO_REFSTATUS) == 'guest' and server.GetGameInfo(GAME_STATE) < 3) or (server.GetClientInfo(index, INFO_REFSTATUS) == 'normal') or (server.GetClientInfo(index, INFO_REFSTATUS) == 'god'):
    ...
    if (server.GetClientInfo(index, INFO_REFSTATUS) == 'guest' and server.GetGameInfo(GAME_STATE) >= 3):
        server.Notify(index,'^yGuest ^yreferee ^ycommands ^yare ^ynot ^yallowed ^yduring ^ythe ^ymatch! ^yWait ^yuntil ^ythe ^ynext ^ysetup ^ytime.')
...
def senddenymessage(index):
    if (server.GetClientInfo(index, INFO_REFSTATUS) == 'guest' and server.GetGameInfo(GAME_STATE) >= 3):
        ...
        server.Notify(index,'^yYou ^yare ^ynot ^ya ^yreferee.  ^yTo ^ybecome ^yreferee, ^ytype ^y"/refpwd ^y<password>"')

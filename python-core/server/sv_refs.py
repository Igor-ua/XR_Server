...
validItems = ['ammo_box', 'arrow_tower_weapon', 'ballista_weapon', 'barrel_explode', 'beast_arcana', 'beast_behemoth',
              'beast_behemoth_melee', 'beast_camouflage', 'beast_charmshrine', 'beast_entropy_shrine',
              ...
              'spire_weapon', 'stables', 'wall_prop', 'wiskeygulch_bridge']
...
def help(index):
    ...
    if (server.GetClientInfo(index, INFO_REFSTATUS) == 'guest' and server.GetGameInfo(GAME_STATE) >= 3):
        server.Notify(index,
                      '^yGuest ^yreferee ^ycommands ^yare ^ynot ^yallowed ^yduring ^ythe ^ymatch! ^yWait ^yuntil ^ythe ^ynext ^ysetup ^ytime.')

...
#    Description: Basic implemetation of Bots in Python (for demonstration)
#---------------------------------------------------------------------------
...
core.CvarSetValue('sv_botCheckDodgeRangeMin', 200)  # When fighting another client, Bots dodge continuously if they are further from target than this value
core.CvarSetValue('sv_botCheckDodgeRangeMax', 900)  # When fighting another client, Bots dodge continuously if they are closer from target than this value

core.CvarSetValue('sv_botWeaponRangeMin', 100)      # Minimum Distance to Target at which Ranged Weapons can be used
core.CvarSetValue('sv_botWeaponAccuracyAng', 5)     # Angular accuracy (degs) of ranged weapon shots (the lower the more accurate)
core.CvarSetValue('sv_botHumanDisruptorDist', 550)  # Min. Distance to Enemy Building before using Disruptor
core.CvarSetValue('sv_botHumanDisruptorDelay', 2500)# Delay (msec) before bot starts moving again after throwing Disruptor
...
core.CvarSetValue('sv_botStaminaCostPercent', 25)   # Stamina cost percentage (integer) stamina -= staminaCost * percent / 100
...
core.CvarSetValue('sv_botPersonaName', 0)           # True/False to determine if a Bot should use a real name or a Persona Level name

core.CvarSetValue('sv_botRandomPersona', 1)         # Randomize Bot Persona at join/add - 0 = off, 1 = on, 2 = selected pool (TODO)
...
def init():
    ...
    core.ConsolePrint('Python: Initializing Bots...\n')
    ...
    try:    
        ...
        # Print Error Message
        core.ConsolePrint('Python: Initialization FAILED!\n')
...
def add(botIndex,botTeam,botRaceDesc):
    ...
    # Print Console Message
    core.ConsolePrint('Python: Allocating Bot Information...\n')
    ...
    try:
        ...
        if botRaceDesc == 'The Legion of Man':
            ...
            if core.CvarGetValue('sv_botRandomPersona') == 1:
                skillLevel = random.randint(0, 12)
            ...
            if core.CvarGetValue('sv_botRandomPersona') == 1:
                skillLevel = random.randint(0, 20)
            elif core.CvarGetValue('sv_botRandomPersona') == 2:
                skillLevel = random.randint(0, 3)
            else:
        ...
        # Print Error Message
        core.ConsolePrint('Python: Allocation FAILED!\n')
...
def frame_playing_checkFSM():
    ...
    if botMission == 0: # Destroy the enemy base
        ...
        core.ConsolePrint('We need more gold\n')
        # Try to look for Critters (fast money, look as far as 2x the distance normally allowed)
        critterIndex = sv_utils.find_nearest_critter(botIndex,2*core.CvarGetValue('sv_botCheckCritterRange'),objDibber)
        if critterIndex > -1:
            core.ConsolePrint('Found critter\n')
        ...
        if botMoney < core.CvarGetValue('sv_botCheckCritterGold'):
            critterIndex = sv_utils.find_nearest_critter(botIndex,core.CvarGetValue('sv_botCheckCritterRange'),objDibber)
            if critterIndex > -1:
                # Dibs on that Critter!
                set_dibs(botIndex,critterIndex)
                bot.NavSetTargetObj(botIndex,critterIndex,ORDER_ATTACK)
...
def frame_playing_checkInput():
    ...
    if disableNav == 0:
        ...
        if botState == STATE_MOVING and sv_defs.gameTime > immobilizedTime[botIndex]:
            if tgtIndex > -1 and (tgtDist <= 2.5 * bot.GetInventRange(botIndex,0) or core.CvarGetValue('sv_duelserver')):
    ...
    # Objective: Attack Something
        ...
        if tgtIndex > -1:
            ...
            # Is target a client?                    
                ...
                if tgtDist >= core.CvarGetValue('sv_botWeaponRangeMin') and bot.GetInventType(botIndex,1) == INVTYPE_RANGED and bot.GetInventReady(botIndex,1):
                ...
                # Use Melee once in Range
                        ...
                        if randomTauntTimer >= int(core.CvarGetValue('sv_botRandomTauntFreq')):
                            if int(core.CvarGetValue('sv_botTalkFreq')) > 0:
                                ...
                                if rnd <= int(core.CvarGetValue('sv_botTalkFreq')):
                                    ...
                                    randomTauntTimer = 0 - random.randint(0,int(core.CvarGetValue('sv_botRandomTauntFreq')/3))
                    ...
                    targetHealth = float(sv_defs.clientList_Health[tgtIndex])/float(sv_defs.clientList_MaxHealth[tgtIndex])
                    ...
                    if raceDesc == 'The Legion of Man':
                        ...
                        if type == 0:
                            ...
                            human_delayed_block_skilled(botIndex, targetWState, botWState, health, targetHealth, stamina, tgtDist)
                        elif type < 0:
...
def frame_playing_checkDodge():
    ...
    if lastCheckDodge[botIndex] + core.CvarGetValue('sv_botCheckDodgeFreq') < sv_defs.gameTime:
        ...
        if botGoal == GOAL_ATTACKMELEE and tgtIndex > -1 and sv_utils.is_client(tgtIndex) and tgtDist < core.CvarGetValue('sv_botCheckDodgeRangeMax'):
...
def frame_playing_checkItems():
    ...
    if raceDesc == 'The Legion of Man':
        ...
        # Need Medkit boost?
        slot = bot.SearchInvent(botIndex,'human_medkit')
        if slot >= 0 and (float(sv_defs.clientList_Health[botIndex])/float(sv_defs.clientList_MaxHealth[botIndex])) < 0.6 and \
        ...
        if slot >= 0 and botGoal == GOAL_ATTACKMELEE and sv_utils.is_building(tgtIndex) and bot.NavTargetVisible(botIndex,demoDist):
            activate_item(botIndex,slot)
            
        # Try to use Disruptor on Spires
        global immobilizedTime
        slot = bot.SearchInvent(botIndex,'human_disruptor')
        if slot >= 0 and botGoal == GOAL_ATTACKMELEE and tgtDist < core.CvarGetValue('sv_botHumanDisruptorDist') and sv_defs.objectList_Name[tgtIndex] == 'beast_fire_spire':            
        ...
        if slot >= 0 and botGoal == GOAL_ATTACKMELEE and sv_utils.is_building(tgtIndex) and tgtDist < core.CvarGetValue('sv_botBeastProtectDist'):
            activate_item(botIndex,slot)
        
        # Try using Sacrifice near Buildings
        slot = bot.SearchInvent(botIndex,'beast_immolate')
        if slot >= 0 and botGoal == GOAL_ATTACKMELEE and sv_utils.is_building(tgtIndex) and tgtDist < core.CvarGetValue('sv_botBeastSacrificeDist'):
            isSaccing = 1
            activate_item(botIndex,slot)
...
def beast_bobbot(botIndex, targetWState, botWState, health, targetHealth, stamina, tgtDist):
    ...
    if tgtDist <= 5.0 * bot.GetInventRange(botIndex,0):
        if bobbotTimer < 3 and targetWState == 1 and botWState == 0 and tgtDist <= 1.5 * bot.GetInventRange(botIndex,0):
            Bleap_B(botIndex)
            bobbotTimer = 1
            return
        if tgtDist < 1.0 * bot.GetInventRange(botIndex,0):
...
def beast_aim(botIndex, targetWState, botWState, health, targetHealth, stamina, tgtDist):
    ...
    if targetWState == 17 and botWState > 0 and botWState < maxHits+2 and difAngle < 110 and tgtDist <= 1.0 * bot.GetInventRange(botIndex,0):
...
def beast_pro(botIndex, targetWState, botWState, health, targetHealth, stamina, tgtDist):
    ...
    if targetWState > 0 and targetWState < 5 and health < 1.0:
        ...
        if tgtDist <= 1.3 * bot.GetInventRange(botIndex,0):
            ...
            if botWState < maxHits or (botWState < maxHits+1 and health > 0.4) or (targetWState == 0 or targetWState == -1 or targetWState == 17):
                Battack_leap_F(botIndex)
            else:
                Bmove_F(botIndex)
    ...
    if targetWState == 17 and botWState > 0 and botWState < maxHits+2 and difAngle < 110 and tgtDist <= 1.1 * bot.GetInventRange(botIndex,0):
        if boolean1 == 0:
            angle = (math.atan2(-tgtDir[0],tgtDir[1])*(180.0)/3.1415)+110.0
        else:
            angle = (math.atan2(-tgtDir[0],tgtDir[1])*(180.0)/3.1415)-110.0
        ...
        #else:
        if boolean1 == 0:
            angle = (math.atan2(-tgtDir[0],tgtDir[1])*(180.0)/3.1415) - ((tgtDist/bot.GetInventRange(botIndex,0)*20))
        else:
            angle = (math.atan2(-tgtDir[0],tgtDir[1])*(180.0)/3.1415) + ((tgtDist/bot.GetInventRange(botIndex,0)*20))
        bot.SetAngle(botIndex,ANGLE_YAW,angle)
